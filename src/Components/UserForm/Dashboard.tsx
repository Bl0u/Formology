...
const Dashboard = () => {
  ...
  const [selectedFormEmails, setSelectedFormEmails] = useState<{ email: string; formId: string }[]>([]); // Emails with associated form IDs
  ...
  const handleUserResponse = async (formId: string, email: string) => {
    try {
      ...
      const response = await axios.post("http://localhost/Form/UserResponse.php", {
        formId,
        email,
      });
      ...
      console.error(`Failed to fetch user response for Form ID ${formId} and Email ${email}:`, error);

      // Handle different types of errors
      if (error.response) {
        alert(`Error: ${error.response.data.message || "Failed to fetch the user response."}`);
    ...
    <>
    {
        ...
        >
          {forms.map((form) => (
            <motion.div
              ...
              whileTap={{ scale: 0.95 }}
            >
              <span className="form-id-text">{form.first_section_title}</span>
              <button
      ...
      <div className="right-panel">
        {responseDetails ? (
          ...
          <p className="placeholder-text">Select an email to view responses</p>
        )}
      </div>
    </div>
      )
      : (
        <IdleRedirect isLogged={isLogged}/>
      )
    }
    </>
  );
